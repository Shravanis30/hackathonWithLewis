...
const messages = [
    ...
    "Hmm, think again?",
    "That choice seems risky.",
    "Maybe look at it again?",
    "You sure about this?",
    "That’s an odd pick.",
    "Not too sure about that.",
    "Could be wrong, huh?",
    "Careful, might be tricky!"
];

function displayRandomMessage() {
    const randomMessage = messages[Math.floor(Math.random() * messages.length)];
    const messageElement = document.getElementById('random-message-bottom-right');

    if (messageElement) {
        messageElement.textContent = randomMessage;
        messageElement.style.display = 'flex'; // Show message

        // Play the sound
        const sound = document.getElementById('confusingSound');
        sound.play();
...
// Example event listener for number buttons
document.querySelectorAll('.number').forEach(button => {
    button.addEventListener('click', () => {
        // Assuming you have a function to handle filling cells
        fillCellWithNumber(button.textContent);
        displayRandomMessage();
    });
});

// Placeholder function for filling cells
function fillCellWithNumber(number) {
    // Implement your logic for filling cells here
    console.log(`Cell filled with number: ${number}`);
}
...
const startShuffleTimer = () => {
    clearTimeout(shuffleTimer); // Clear any existing timer
...
const shuffleBoard = () => {
    su = sudokuGen(level); // Generate a new puzzle
...
const displaySudoku = () => {
    for (let i = 0; i < Math.pow(CONSTANT.GRID_SIZE, 2); i++) {
        ...
        if (su.question[row][col] !== 0) {
            ...
            cells[i].innerHTML = ''; // Clear cell content for unfilled cells
        }
    }
};
...
const  result_message = document.querySelector('#result-time');
    ...
    number_inputs.forEach((e, index) => {
        e.addEventListener('click', () => {
            if (!cells[selected_cell].classList.contains('filled')) {
                ...
                // Restart the shuffle timer after a valid input
                startShuffleTimer();
            }
        });
    });
    ...
    cells.forEach((e, index) => {
        e.addEventListener('click', () => {
            if (!e.classList.contains('filled')) {
                ...
                // Start the shuffle timer when a cell is selected
                startShuffleTimer();
            }
        })
    })
